...
export async function POST(request: NextRequest) {
  try {
    ...
    const twitterHandle = body.twitterHandle ? String(body.twitterHandle).toLowerCase() : null;
    const walletAddress = body.walletAddress ? String(body.walletAddress) : null;
    const privyId = body.privyId ? String(body.privyId) : null;
    const source = body.source ? String(body.source).slice(0, 64) : "post-login-modal";
    ...
    const [emailRes, tgRes] = await Promise.allSettled([
      sendEmail({ to: email, subject: welcome.subject, html: welcome.html, text: welcome.text }),
      sendTelegramMessage(tgLines.join("\n")),
    ]);

    if (emailRes.status === "rejected" || (emailRes.status === "fulfilled" && emailRes.value.error)) {
      console.error("Resend send failed:", emailRes.status === "rejected" ? emailRes.reason : emailRes.value.error);
    }
    if (tgRes.status === "rejected" || (tgRes.status === "fulfilled" && !tgRes.value.ok)) {
      console.error("Telegram send failed:", tgRes.status === "rejected" ? tgRes.reason : tgRes.value.error);
    }
